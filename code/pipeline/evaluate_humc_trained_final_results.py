...
def main() -> None:
    ...
    HUMC_INFERENCE_DIR.mkdir(parents=True, exist_ok=True)

    norm_stats = load_norm_stats()
    eval_sets = {"HUMC_Test": load_humc_test()}
    ...
    for backbone in args.models:
        for aug_name in args.augmentations:
            run_output = HUMC_INFERENCE_DIR / f"{backbone}_{aug_name}"
            for fold_id in args.folds:
                ...
                pd.DataFrame(foldwise_rows).to_csv(
                    HUMC_INFERENCE_DIR / "__ALL_foldwise_results.csv",
                    index=False,
                )
    ...
    foldwise_df.to_csv(HUMC_INFERENCE_DIR / "__ALL_foldwise_results.csv", index=False)
    summary_df.to_csv(HUMC_INFERENCE_DIR / "__ALL_summary_results.csv", index=False)

    with open(HUMC_INFERENCE_DIR / "__evaluation_meta.json", "w", encoding="utf-8") as f:
        json.dump({
