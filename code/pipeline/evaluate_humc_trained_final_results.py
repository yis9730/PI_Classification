"""Evaluate HUMC-trained checkpoints internally and on external datasets.

HUMC images, patient identifiers, and image-level split tables remain private.
Run this script only inside an environment authorized to use those data.

    - HUMC_Test: held-out 15% patient-level internal test set
    - PIID: public external validation set
...
EXPECTED_PIID_IMAGES = 1081
EXPECTED_KAGGLE_IMAGES = 141
EXPECTED_HUMC_TEST_IMAGES = 288
EXPECTED_HUMC_TRAINVAL_IMAGES = 1556
EXPECTED_HUMC_TRAINVAL_PATIENTS = 425
EXPECTED_HUMC_TEST_PATIENTS = 75
EXPECTED_PIID_STAGE_COUNTS = (229, 311, 273, 268)
EXPECTED_KAGGLE_STAGE_COUNTS = (27, 46, 41, 27)
EXPECTED_HUMC_TEST_STAGE_COUNTS = (30, 104, 100, 54)
EXPECTED_HUMC_TOTAL_STAGE_COUNTS = (233, 709, 575, 327)
EXPECTED_NORMALIZATION_TRAIN_IMAGES = (1251, 1269, 1235, 1171, 1298)
...
def _validate_normalization_dataframe(
    ...
    if "n_train_images" in numeric.columns:
        ...
        if len(observed) != N_FOLDS or observed != expected:
            raise ValueError(
                "Normalization n_train_images does not match the released HUMC folds"
            )
...
def training_input_sha256() -> dict[str, str]:
    paths = {
        ...
        "fold_indices_json": HUMC_SPLIT_DIR / "fold_indices.json",
        "normalization_stats_csv": HUMC_SPLIT_DIR / "normalization_stats.csv",
    }
    missing = [path for path in paths.values() if not path.is_file()]
    if missing:
        raise FileNotFoundError(f"HUMC training split artifacts are missing: {missing}")
    return {name: sha256_file(path) for name, path in paths.items()}


def load_norm_stats() -> dict[int, dict[str, list[float]]]:
    path = HUMC_SPLIT_DIR / "normalization_stats.csv"
    if not path.exists():
        raise FileNotFoundError(f"HUMC normalization file not found: {path}")
    return _validate_normalization_dataframe(pd.read_csv(path))


def _validate_humc_split_metadata() -> None:
    path = HUMC_SPLIT_DIR / "split_meta.json"
    if not path.is_file():
        return
    try:
        meta = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ValueError(f"Invalid HUMC split metadata: {path}") from exc
    expected_scalars = {
        "dataset": "HUMC",
        "random_seed": RANDOM_SEED,
        "n_folds": N_FOLDS,
        "total_images": sum(EXPECTED_HUMC_TOTAL_STAGE_COUNTS),
        "total_patients": 500,
        "n_trainval_images": 1556,
        "n_trainval_patients": 425,
        "n_test_images": EXPECTED_HUMC_TEST_IMAGES,
        "n_test_patients": 75,
    }
    mismatches = {
    ...
    if not isinstance(normalization, dict) or normalization.get("input_size") != INPUT_SIZE:
        mismatches["normalization.input_size"] = {
            "expected": INPUT_SIZE,
            "found": normalization.get("input_size") if isinstance(normalization, dict) else None,
        }
    stage_distribution = meta.get("stage_distribution", {})
    expected_distributions = {
        "total": EXPECTED_HUMC_TOTAL_STAGE_COUNTS,
        "test": EXPECTED_HUMC_TEST_STAGE_COUNTS,
    }
    ...
    if mismatches:
        raise ValueError(f"HUMC split metadata does not match the study contract: {mismatches}")


def load_humc_test() -> tuple[list[str], list[int]]:
    path = HUMC_SPLIT_DIR / "test_set.csv"
    if not path.exists():
        raise FileNotFoundError(f"Private HUMC test split file not found: {path}")
    _validate_humc_split_metadata()
    df = pd.read_csv(
        path,
        dtype={"image_path": "string", "file_stem": "string", "patient_id": "string"},
    )
    if not {"image_path", "stage", "patient_id"} <= set(df.columns):
        raise ValueError("HUMC test CSV must contain image_path, stage, and patient_id columns")
    if df[["image_path", "stage", "patient_id"]].isna().any().any():
        raise ValueError("HUMC test CSV contains missing required values")
    if df["image_path"].str.strip().eq("").any() or df["patient_id"].str.strip().eq("").any():
        raise ValueError("HUMC test CSV contains an empty image_path or patient_id")
    ...
    if numeric_stage.isna().any() or not np.all(numeric_stage == np.floor(numeric_stage)):
        raise ValueError("HUMC test stages must be integer labels 1..4")
    if not numeric_stage.astype(int).isin(range(1, NUM_CLASSES + 1)).all():
        raise ValueError("HUMC test stages must be in 1..4")
    paths = resolve_project_paths(df["image_path"].tolist())
    test_patients = set(df["patient_id"].astype(str).str.strip())
    if len(test_patients) != EXPECTED_HUMC_TEST_PATIENTS:
        raise ValueError(
            f"HUMC test CSV must contain exactly {EXPECTED_HUMC_TEST_PATIENTS} patients"
        )
    trainval_path = HUMC_SPLIT_DIR / "trainval_set.csv"
    trainval = pd.read_csv(
        trainval_path,
        dtype={"image_path": "string", "patient_id": "string"},
    )
    if (
        not {"image_path", "patient_id"} <= set(trainval.columns)
        or len(trainval) != EXPECTED_HUMC_TRAINVAL_IMAGES
    ):
        raise ValueError(
            "HUMC trainval CSV must contain image_path, patient_id, and exactly "
            f"{EXPECTED_HUMC_TRAINVAL_IMAGES} rows"
        )
    if trainval[["image_path", "patient_id"]].isna().any().any():
        raise ValueError("HUMC trainval CSV contains missing image_path or patient_id values")
    if (
        trainval["image_path"].str.strip().eq("").any()
        or trainval["patient_id"].str.strip().eq("").any()
    ):
        raise ValueError("HUMC trainval CSV contains an empty image_path or patient_id")
    trainval_patients = set(trainval["patient_id"].astype(str).str.strip())
    if len(trainval_patients) != EXPECTED_HUMC_TRAINVAL_PATIENTS:
        raise ValueError(
            f"HUMC trainval CSV must contain exactly {EXPECTED_HUMC_TRAINVAL_PATIENTS} patients"
        )
    if test_patients & trainval_patients:
        raise ValueError("HUMC trainval and held-out test patients overlap")
    trainval_keys = {
    ...
    if test_keys & trainval_keys:
        raise ValueError("HUMC trainval and held-out test image paths overlap")
...
def checkpoint_path(
    ...
    return HUMC_CHECKPOINT_DIR / run_name / "best_models_weights" / f"best_model_fold_{fold_id}.pth"
...
def _checkpoint_rng_contract(
    ...
    return "per_fold_sha256_v1", derive_fold_seed(
        run_digest, fold_id, dataset="HUMC"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate HUMC-trained checkpoints.")
...
def main() -> None:
    ...
    HUMC_INFERENCE_DIR.parent.mkdir(parents=True, exist_ok=True)
    staging_context = tempfile.TemporaryDirectory(
        prefix=f".{HUMC_INFERENCE_DIR.name}-staging-",
        dir=HUMC_INFERENCE_DIR.parent,
    )
    ...
    humc_test = load_humc_test()
    split_metadata_present = (HUMC_SPLIT_DIR / "split_meta.json").is_file()
    require_dataset_contract(
        "HUMC_Test", humc_test, EXPECTED_HUMC_TEST_STAGE_COUNTS
    )
    eval_sets = {"HUMC_Test": humc_test}
    piid = load_stage_folder_dataset(PIID_DATA_DIR)
    require_dataset_contract("PIID", piid, EXPECTED_PIID_STAGE_COUNTS)
    kaggle = load_stage_folder_dataset(KAGGLE_DATA_DIR)
    require_dataset_contract("Kaggle", kaggle, EXPECTED_KAGGLE_STAGE_COUNTS)
    eval_sets["PIID"] = piid
    eval_sets["Kaggle"] = kaggle
    ...
    manuscript_complete = (
        ...
        and set(eval_sets) == {"HUMC_Test", "PIID", "Kaggle"}
    ...
    _atomic_write_json(meta_path, {
            ...
            "normalization_source": str(HUMC_SPLIT_DIR / "normalization_stats.csv"),
            "training_input_sha256": input_file_sha256,
            "checkpoint_root": str(HUMC_CHECKPOINT_DIR),
        })

    _promote_snapshot(output_root, HUMC_INFERENCE_DIR)
    staging_context.cleanup()

    print(f"[DONE] Foldwise results: {HUMC_INFERENCE_DIR / '__ALL_foldwise_results.csv'}")
    print(f"[DONE] Summary results: {HUMC_INFERENCE_DIR / '__ALL_summary_results.csv'}")
