...
N_FOLDS = 5
CLASS_NAMES = ["1", "2", "3", "4"]
EXPECTED_TRAINVAL_STAGE_COUNTS = (194, 264, 232, 228)
EXPECTED_TRAINVAL_IMAGES = sum(EXPECTED_TRAINVAL_STAGE_COUNTS)
EXPECTED_FOLD_SIZES = ((734, 184), (734, 184), (734, 184), (735, 183), (735, 183))
...
def _rng_contract(
    ...
    return (
        "per_fold_sha256_v1",
        derive_fold_seed(run_digest, fold_id, dataset="PIID"),
    )
...
def _completed_run_or_none(
    ...
    if overwrite:
        # Invalidate any prior completion before replacing its checkpoint. If
        # this run is interrupted, the remaining weight is treated as orphaned.
    ...
    # Detect truncated or non-state-dictionary files before declaring a skip.
...
def _validate_trainval_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"PIID trainval CSV is missing columns: {sorted(missing)}")
    if len(df) != EXPECTED_TRAINVAL_IMAGES:
        raise ValueError(
            f"PIID trainval CSV must contain {EXPECTED_TRAINVAL_IMAGES} rows; "
            f"found {len(df)}"
        )
...
def _validate_fold_indices(fold_indices: dict, n_rows: int) -> None:
...
def load_split_files() -> tuple[pd.DataFrame, dict, dict, dict[str, str]]:
    ...
    df_trainval = _validate_trainval_dataframe(pd.read_csv(trainval_csv))
    with open(fold_json, "r", encoding="utf-8") as f:
        fold_indices = json.load(f)
    _validate_fold_indices(fold_indices, len(df_trainval))
